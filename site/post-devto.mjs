// Post bài lên Dev.to via API
// Cách dùng:
//   1. Lấy API key: dev.to → Settings → Extensions → DEV API Keys → New Key
//   2. Chạy: DEVTO_API_KEY=your_key node site/post-devto.mjs
//   3. Bài sẽ được đăng ngay (published: true) hoặc lưu draft (published: false)

const API_KEY = process.env.DEVTO_API_KEY

if (!API_KEY) {
  console.error('Thiếu API key. Chạy: DEVTO_API_KEY=your_key node site/post-devto.mjs')
  process.exit(1)
}

const article = {
  title: 'How I built a privacy-first image compressor that runs entirely in the browser',
  body_markdown: `Most online image compressors have one thing in common: your files go to their server. You upload a private photo, it travels across the internet, gets processed on someone else's machine, and comes back. That always felt wrong to me.
...
*Built with Next.js, deployed on Vercel. Questions welcome in the comments.*`,
  ...
  canonical_url: 'https://compressimg.pro/compress-image',
}

async function post() {
  console.log('Đang đăng bài lên Dev.to...')
