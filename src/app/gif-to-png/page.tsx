...
import GifToPngContentSection from '@/components/tool/GifToPngContentSection'
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        setResult(converted)
        setPageState('done')
        analytics.imageConverted('gif', 'png', file.size / 1024, converted.outputSize / 1024)
      } catch {
        setErrorMessage('Conversion failed. Please try again.')
        setPageState('error')
        analytics.convertError('gif_to_png_failed')
      }
    },
    [result]
  )
    ...
    <main className="flex-1">
      <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12 space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-text-main">GIF to PNG Converter</h1>
          <p className="text-text-muted text-sm sm:text-base">
            Convert GIF to PNG instantly — lossless output, transparency preserved, free, 100% in
            your browser
          </p>
        </div>
        ...
        <div className="space-y-4">
          <UploadBox
            ...
            processingLabel="Converting GIF to PNG..."
        ...
        {pageState === 'done' && result && originalFile && (
          <ConvertResultCard
            ...
            originalName={originalFile.name.replace(/\.gif$/i, '.png')}
            onReset={handleReset}
            onDownload={() =>
