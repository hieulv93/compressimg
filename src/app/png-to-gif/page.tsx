...
import PngToGifContentSection from '@/components/tool/PngToGifContentSection'
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      if (fileWithFlag._invalid === 'format') {
        setErrorMessage('Unsupported format. Please upload a PNG file.')
        setPageState('error')
        return
      }
      ...
      try {
        ...
        setResult(converted)
        setPageState('done')
        analytics.imageConverted('png', 'gif', file.size / 1024, converted.outputSize / 1024)
      } catch {
        setErrorMessage('Conversion failed. Please try again.')
        setPageState('error')
        analytics.convertError('png_to_gif_failed')
      }
    },
    [result]
  )
    ...
    <main className="flex-1">
      <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12 space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-text-main">PNG to GIF Converter</h1>
          <p className="text-text-muted text-sm sm:text-base">
            Convert PNG to GIF instantly — free, 100% in your browser
          </p>
        </div>
        ...
        <div className="space-y-4">
          <UploadBox
            ...
            processingLabel="Converting PNG to GIF..."
            accept="image/png,.png"
            formatHint={`PNG — max ${MAX_FILE_SIZE_MB}MB`}
          />
        ...
        {pageState === 'done' && result && originalFile && (
          <ConvertResultCard
            ...
            originalName={originalFile.name.replace(/\.png$/i, '.gif')}
            onReset={handleReset}
            onDownload={() =>
