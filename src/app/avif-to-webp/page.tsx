...
import AvifToWebpContentSection from '@/components/tool/AvifToWebpContentSection'
...
export default function AvifToWebpPage() {
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const converted = await convertImage(file, { outputFormat: 'image/webp' })
        setResult({ ...converted, inputFormat: 'avif', outputExt: 'webp' })
        setPageState('done')
        analytics.imageConverted('avif', 'webp', file.size / 1024, converted.outputSize / 1024)
      } catch {
        setErrorMessage(
          'Conversion failed. Your browser may not support AVIF. Try Chrome 85+ or Firefox 93+.'
        )
        setPageState('error')
        analytics.convertError('avif_to_webp_failed')
      }
    },
    [result]
  )
    ...
    <main className="flex-1">
      <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12 space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-text-main">AVIF to WebP Converter</h1>
          <p className="text-text-muted text-sm sm:text-base">
            Convert AVIF to WebP instantly — smaller than PNG, wider support than AVIF, free, 100%
            in your browser
          </p>
        </div>
        ...
        <div className="space-y-4">
          <UploadBox
            ...
            processingLabel="Converting AVIF to WebP..."
        ...
        {pageState === 'done' && result && originalFile && (
          <ConvertResultCard
            ...
            originalName={originalFile.name.replace(/\.avif$/i, '.webp')}
            onReset={handleReset}
            onDownload={() =>
