...
import HeicToPngContentSection from '@/components/tool/HeicToPngContentSection'
import { isHeicFile, convertHeicToPng } from '@/lib/heic'
...
export default function HeicToPngPage() {
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const blob = await convertHeicToPng(file)
        ...
        setPageState('done')
        analytics.imageConverted('heic', 'png', file.size / 1024, blob.size / 1024)
      } catch {
        setErrorMessage('Conversion failed. Please try again.')
        setPageState('error')
        analytics.convertError('heic_to_png_failed')
      }
    },
    [result]
  )
    ...
    <main className="flex-1">
      <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12 space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-text-main">HEIC to PNG Converter</h1>
        ...
        <div className="space-y-4">
          <UploadBox
            ...
            processingLabel="Converting HEIC to PNG..."
            accept="image/heic,image/heif,.heic,.heif"
            customValidate={isHeicFile}
        ...
        {pageState === 'done' && result && originalFile && (
          <ConvertResultCard
            ...
            originalName={originalFile.name.replace(/\.(heic|heif)$/i, '.png')}
            onReset={handleReset}
            onDownload={() =>
