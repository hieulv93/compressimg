...
import JpgToGifContentSection from '@/components/tool/JpgToGifContentSection'
import { convertToGif } from '@/lib/gif-convert'
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      if (fileWithFlag._invalid === 'format') {
        setErrorMessage('Unsupported format. Please upload a JPG or JPEG file.')
        setPageState('error')
        return
      }
      ...
      try {
        const converted = await convertToGif(file)
        setResult(converted)
        setPageState('done')
        analytics.imageConverted('jpg', 'gif', file.size / 1024, converted.outputSize / 1024)
      } catch {
        setErrorMessage('Conversion failed. Please try again.')
        setPageState('error')
        analytics.convertError('jpg_to_gif_failed')
      }
    },
    [result]
  )
    ...
    <main className="flex-1">
      <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12 space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-text-main">JPG to GIF Converter</h1>
          <p className="text-text-muted text-sm sm:text-base">
            Convert JPG to GIF instantly — free, 100% in your browser
          </p>
        </div>
        ...
        <div className="space-y-4">
          <UploadBox
            ...
            processingLabel="Converting JPG to GIF..."
            accept="image/jpeg,image/jpg,.jpg,.jpeg"
            formatHint={`JPG / JPEG — max ${MAX_FILE_SIZE_MB}MB`}
          />
        ...
        {pageState === 'done' && result && originalFile && (
          <ConvertResultCard
            ...
            originalName={originalFile.name.replace(/\.jpe?g$/i, '.gif')}
            onReset={handleReset}
            onDownload={() =>
