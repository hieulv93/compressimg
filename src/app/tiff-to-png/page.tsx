...
import TiffToPngContentSection from '@/components/tool/TiffToPngContentSection'
import { convertTiffToPng, revokeConvertPreview } from '@/lib/convert'
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        setResult(converted)
        setPageState('done')
        analytics.imageConverted('tiff', 'png', file.size / 1024, converted.outputSize / 1024)
      } catch {
        setErrorMessage('Conversion failed. Please try again with a different TIFF file.')
        setPageState('error')
        analytics.convertError('tiff_to_png_failed')
      }
    },
    [result]
  )
    ...
    <main className="flex-1">
      <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12 space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-text-main">TIFF to PNG Converter</h1>
        ...
        <div className="space-y-4">
          <UploadBox
            ...
            processingLabel="Converting TIFF to PNG..."
        ...
        {pageState === 'done' && result && originalFile && (
          <ConvertResultCard
            ...
            originalName={originalFile.name.replace(/\.tiff?$/i, '.png')}
            onReset={handleReset}
            onDownload={() =>
