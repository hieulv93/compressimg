...
import WebpToGifContentSection from '@/components/tool/WebpToGifContentSection'
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      if (fileWithFlag._invalid === 'format') {
        setErrorMessage('Unsupported format. Please upload a WebP file.')
        setPageState('error')
        return
      }
      ...
      try {
        ...
        setResult(converted)
        setPageState('done')
        analytics.imageConverted('webp', 'gif', file.size / 1024, converted.outputSize / 1024)
      } catch {
        setErrorMessage('Conversion failed. Please try again.')
        setPageState('error')
        analytics.convertError('webp_to_gif_failed')
      }
    },
    [result]
  )
    ...
    <main className="flex-1">
      <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12 space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-text-main">WebP to GIF Converter</h1>
          <p className="text-text-muted text-sm sm:text-base">
            Convert WebP to GIF instantly — free, 100% in your browser
          </p>
        </div>
        ...
        <div className="space-y-4">
          <UploadBox
            ...
            processingLabel="Converting WebP to GIF..."
            accept="image/webp,.webp"
            formatHint={`WebP — max ${MAX_FILE_SIZE_MB}MB`}
          />
        ...
        {pageState === 'done' && result && originalFile && (
          <ConvertResultCard
            ...
            originalName={originalFile.name.replace(/\.webp$/i, '.gif')}
            onReset={handleReset}
            onDownload={() =>
